...
class L1TkMuCorrDynamicWindows{
    ...
    public:
        ...
        std::vector<int> find_match_stub(const EMTFHitCollection& l1mus, const L1TTTrackCollectionType& l1trks, std::vector< std::vector<int> > & matchedStubs, const int& station, bool requireBX0 = true, bool requirePhaseII = false); // gives a vector with the idxs of muon stubs from station "station" for each L1TTT
